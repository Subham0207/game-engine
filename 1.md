...
I have fixed the directory to  project now for txture files. But the the Materials are loading as NULL. So something wrong with boost serialization of them.
    ...
    [PENDING] statemachine UI.
        ...
        [PENDING] saving logic: with new statemachine and existing statemachine.
            [pending] not working for new statemachine; saving errors.
        [Bug] if you save an existing state machine with different name. the new file still uses the old guid. Which essentially overrides the old statemachine in asset browser or the whole engine ( have not done a thorugh check). Pls re-gen guid in those cases.
