...
1. create a enum of UI layers which will include the HUD. We load the .rml, .rcss files here. And then We fire events to decide which UI layer to draw. UI layer listens to UILayerUpdateEvent.
2. Decide if HUD system class is the UILayer. Is every UI a HUD ? is Pause Menu a HUD ?
...
2. Use HUD to show health bar. And player can damage the AI enemies by jumping on top of them.
3. Make a small demo game then.
4. Character Editor UI: To add colliders ( adjust position ), parent the collider to a bone with offset.