...
7. [PENDING] Later add option to modify collider shape and its type.
   ...
   [LETS ACTUALLY UPDATE THE PHYSICS OBJECT TYPE to have be this way. SINCE it the physics object that is actually responsible, But that type is a bit entanged right now, due to capsuleCollider inherting it. Lets figure it out]
    ...
    The type is PhysicsColliderSettings because the actual PhysicsObject instance will be created with these settings.
    [VERIFY PLAN WITH AN AI FIRST]
    a. Add a type PhysicsColliderSettings: { collisionType, ColliderShape, CustomColliderShapeData (optional), transformation (defaults to 0 position, 0 quat, 1.0 for scale in all axis)}. Then Add this type as member variable in Model class. We will remove physicsBodyType and replace it with this. transformation is used to store offset of the collider from the model.
    b. Update model file editor: Add option to update collisionType (None, static, dynamic, kinematic) and ColliderShape (different default shapes supported by JOLT, and a custom shape option).
    c. Need to update the serialization logic to add PhysicsColliderSettings as member of Model. [BREAKING CHANGE, BUT ITS FINE SINCE WE ARE IN EARLY STAGES]
    ...
    e. updating the collision type and physicsBodyType in model file editor will require a restart of game engine to take effect.
    f. physicsBodyType set to custom shape will require the shape be provided. We use CustomColliderShapeData field for this, it should be optional field.
