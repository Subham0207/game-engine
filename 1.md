...
5. Update Character Base logic. Make if isPlay or not always joltCharacter is the one that is moved. Then copy Transformation to 3D model + offset.
   Right now, in Capsule Collider class we create and delete the characterVirtual class based on is IsPlay is enabled. I have moved this to constructor so we always have characterVirtual initilaized.
    game builds but the character is unable to move... will see why.
6. Saving Level
