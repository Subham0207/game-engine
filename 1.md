...
21. [DONE] Creating Animation Editor and named regions
...
## Trigger and Consume Animation Events
1. Trigger Event from Animation with a prefix of character instanceId. So we know which character this animation is for. 
2.  This is good since we will use queue of the window. But then every character will get the event.
2. Still need to decide who will trigger events such that we do not need to check for every tick if there is an Event.
