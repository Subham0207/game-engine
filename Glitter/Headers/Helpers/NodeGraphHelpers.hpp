...
namespace NodeGraphHelpers
{
    ...
    template <typename T, std::size_t... I>
    std::vector<std::string> get_float_field_names_impl(std::index_sequence<I...>)
    {
        std::vector<std::string> names;
    ...
    // Return only reflected float field names from T, preserving struct order.
    template <typename T>
    std::vector<std::string> get_float_field_names()
    {
        return get_float_field_names_impl<T>(
            std::make_index_sequence<boost::pfr::tuple_size_v<T>>{}
        );
    }
