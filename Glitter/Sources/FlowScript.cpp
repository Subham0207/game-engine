...
const std::string& FlowScript::compile()
{
    ...
    try
    {
        const Flowscript::Compile::Ast ast(nodes, nodeGraphLinks);
        Flowscript::Compile::LuaTranspiler transpiler;
        const auto transpiled = transpiler.Transpile(ast.programRoot);

        if (!transpiled.luaCode.empty())
    ...
    {
        compileDiagnostics.push_back(ex.what());
        compiledLua = "-- Generated by FlowScript::compile()\n";
    }
